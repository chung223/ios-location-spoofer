...
## 十三、当 App 用 & 一键改定位（可选，推荐）

部署好之后，还能让「改定位」更像用 App：

1. **当 App 用（PWA）**：用 iPhone 浏览器打开 `https://你的域名/?token=你的TOKEN`，点 Safari 底部「分享」→「添加到主屏幕」。之后从主屏幕图标打开就是全屏独立窗口（本 Worker 已内置图标 `/icon-180.png` 与 apple 元信息，无需额外配置）。
2. **一键改定位（iOS 快捷指令）**：不想每次开网页，可以用「快捷指令」直接打 `POST /set` 改坐标，再切 Wi-Fi/飞行模式触发生效，绑到主屏幕图标或「轻点背面」。完整做法见 [`../快捷指令一键改定位.md`](../快捷指令一键改定位.md)。

> 说明：网页/快捷指令都不能直接改 iOS 的 GPS，真正改定位的是代理模块在半路改写 Apple 定位响应；这里做的是「更方便地设置目标坐标」。改完仍需触发一次重新定位（关开定位服务，或用快捷指令切 Wi-Fi/飞行模式）。
